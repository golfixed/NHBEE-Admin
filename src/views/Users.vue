  ...
  <div class="isdesktop">
    ...
    <table>
      ...
      <tr v-for="(user, i) in userList" :key="'user_' + user.id">
        ...
        <td>{{ user.username }}</td>
        <td>{{ user.name }}</td>
        <td><i :class="'fas fa-' + (user.admin ? 'check' : 'times')"></i></td>
        <td>
...
export default {
  ...
  methods: {
    getUser () {
      ...
      this.isLoading = true
      axios('/admin/user').then((response) => {
        this.userList = response.data
      }).catch((error) => {
        if (error.response && error.response.data) console.error('get user', error.response.data.error)
    ...
    deleteUser (index) {
      if (confirm(`ยืนยันการลบ ${this.userList[index].username} ?`)) {
        axios({
          ...
          if (error.response && error.response.data) console.error('delete user', error.response.data.error)
